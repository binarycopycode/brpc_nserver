...
void HandleTransfileResponse(brpc::Controller *cntl, nserver::fileResponse *resp)
{
    // std::unique_ptr makes sure cntl/response will be deleted before returning.
    std::unique_ptr<brpc::Controller> cntl_guard(cntl);
    std::unique_ptr<nserver::fileResponse> response_guard(resp);
...
int async_get_file()
{
    ...
    nserver::DatanodeService_Stub stub(&channel);


    nserver::fileResponse* response = new nserver::fileResponse();
    brpc::Controller* cntl = new brpc::Controller();
    nserver::fileRequest request;
...
int sync_get_all_date()
{
    ...
    nserver::alldateRequest req;
    nserver::alldateResponse resp;
    brpc::Controller cntl;


    req.set_load_data(true);
    stub.GetAlldate(&cntl, &req, &resp, nullptr);

    if (!cntl.Failed())
    {
        std::cout << "in sync_get_all_date() resp.alldate_size(): " << resp.alldate_size() << std::endl;
        for(int i = 0; i < resp.alldate_size(); i++)
        {
            std::cout << resp.alldate(i) << std::endl;
        }
    }
    else
    {
...
int main(int argc, char* argv[]) 
{
    ...
    if(async_get_file() < 0)
    {
        LOG(ERROR) << "Fail to async_get_file";
    }
    return 0;
}
